...
TEST_P(CustomClientName, UserAgentTest) {
    ...
    if (cluster_nodes > 1) {
        std::string cluster_name_query
            = "SELECT cluster FROM system.clusters WHERE cluster NOT LIKE 'all_groups.%' LIMIT 1";
        std::string cluster_name = *singleStringQuery(cluster_name_query);
        flush_log_query = "SYSTEM FLUSH LOGS ON CLUSTER " + cluster_name;
