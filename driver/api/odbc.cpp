...
#define CASE_STRING(NAME, VALUE) \
            ...
            CASE_NUM(SQL_SQL92_DATETIME_FUNCTIONS, SQLUINTEGER, 0
                     // None of these functions are supported by ClickHouse natively. Instead
                     // {fn ...} style function must be used
            ...
            CASE_NUM(SQL_SQL92_STRING_FUNCTIONS,
                SQLUINTEGER,
                // SQL_SSF_CONVERT | // Not supported by ClickHouse
                SQL_SSF_LOWER |
                SQL_SSF_UPPER |
                SQL_SSF_SUBSTRING |
                // SQL_SSF_TRANSLATE | // Not supported by ClickHouse
                SQL_SSF_TRIM_BOTH |
...
SQLRETURN SQL_API EXPORTED_FUNCTION(SQLRowCount)(HSTMT statement_handle, SQLLEN * out_row_count) {
    ...
    return CALL_WITH_TYPED_HANDLE(SQL_HANDLE_STMT, statement_handle, [&](Statement & statement) {
        if (out_row_count) {
            *out_row_count = statement.getDiagHeader().getAttrAs<SQLLEN>(SQL_DIAG_ROW_COUNT, -1);
