...
void run_test(nanodbc::string const& connection_string)
{
...

    {
        ...
        {
            auto results = execute(connection, NANODBC_TEXT("SELECT `test`.`strings`.`str` AS `platform`, SUM(`test`.`strings`.`id`) AS `sum_installs_ok` FROM `test`.`strings` GROUP BY `str`"));
            show(results);
        }
    ...
    if (0) // Not supported. TODO.
    {
        ...
        execute(
            connection, NANODBC_TEXT("create table batch_test (x varchar, y int, z float) engine Log;"));
        prepare(
