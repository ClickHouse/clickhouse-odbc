...
|      `SSLMode`      |                                                          empty                                                           | One of: `allow`, `prefer`, `require`, use `allow` to enable [`SSL_VERIFY_PEER`](https://www.openssl.org/docs/manmaster/man3/SSL_CTX_set_verify.html) TLS/SSL certificate verification mode, `SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT` is used otherwise |
